...
from ..lanczos import lanczos_resample_two


def test_lanczos1_resample_two_smoke():
    ...
    row = 5.5
    col = 7.5
    val1, val2, _ = lanczos_resample_two(
        im1,
        im2,
...
def test_lanczos2_resample_two_smoke():
    rng = np.random.RandomState(seed=10)
    im1 = rng.normal(size=(11, 25))
    im2 = rng.normal(size=(11, 25))
...
def test_lanczos3_resample_two_smoke():
    rng = np.random.RandomState(seed=10)
    im1 = rng.normal(size=(11, 25))
    im2 = rng.normal(size=(11, 25))
...
def test_lanczos_resample_two_out_of_bounds(row, col):
    ...
    val1, val2, edge = lanczos_resample_two(
        im1,
        im2,
        np.array([row], dtype=np.float64),
        np.array([col], dtype=np.float64),
