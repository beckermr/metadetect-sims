...
def gal_match(pos_det, pos_sim, shear_sim):
    ...
    for i in range(len(gal_det)):
        galaxy = gal_det[i]
        idx = tree.query((galaxy[0], galaxy[1]))
        # use query to get the cloest match
        index.append(idx[1])
        shear_det.append(shear_sim[idx[1]])

    index, shear_det = np.array(index), np.array(shear_det)
    return index, shear_det


def make_catalog(res, pos_sim, shear_sim, pop_index):

    dic = {}
    _, shear_det = gal_match(res['noshear'], pos_sim, shear_sim)
    dic['noshear'] = res['noshear'][np.where((shear_det == pop_index),
                                    True, False)]
    _, shear_det = gal_match(res['1p'], pos_sim, shear_sim)
    dic['1p'] = res['1p'][np.where((shear_det == pop_index), True, False)]
    _, shear_det = gal_match(res['1m'], pos_sim, shear_sim)
    dic['1m'] = res['1m'][np.where((shear_det == pop_index), True, False)]
    _, shear_det = gal_match(res['2p'], pos_sim, shear_sim)
    dic['2p'] = res['2p'][np.where((shear_det == pop_index), True, False)]
    _, shear_det = gal_match(res['2m'], pos_sim, shear_sim)
...
def _run_sim(seed):
    ...
    try:
        ...
        sel = np.random.RandomState(seed=seed+1000000)
        ...
        sim = SIM_CLASS(rng=rng, sel=sel, frac_ex=0.5, **CONFIG)

        mbobs, pos_sim, shear_sim = sim.get_mbobs(return_truth_cat=True)
        md = Metadetect(config, mbobs, rng)
        md.go()

        if match:
            gal = make_catalog(md.result, pos_sim, shear_sim, 1)
            pres = _meas_shear(gal)
            gal_ex = make_catalog(md.result, pos_sim, shear_sim, 0)
