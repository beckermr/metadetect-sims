...
from config_wl import CONFIG
from config_wl import match, invert_ex
from run_preamble import get_shear_meas_config

import os
os.environ['CATSIM_DIR'] = '/home/ac.kzhang58/work/run_sims_redshift/CATSIM_DIR'
from astropy.io import fits
hdul = fits.open('/home/ac.kzhang58/work/run_sims_redshift/CATSIM_DIR/OneDegSq.fits')
z_red = list(hdul[1].data['redshift'])
...
def gal_match(pos_det, truth_cat):
    ...
    for i in range(len(gal_det)):
        galaxy = gal_det[i]
        idx = tree.query_ball_point((galaxy[0], galaxy[1]),3)
        # use query to get the cloest match
        if len(idx) >= 1:
            index.append(idx[0])
            shear_det.append(z_population[idx[0]])
        else:
            index.append(-1)
            shear_det.append(-1)    
...
def make_catalog(res, truth_cat, pop_index):
    ...
    dic['noshear'] = res['noshear'][np.where((shear_det == pop_index), True, False)]  
...
def _run_sim(seed):
    ...
    try:
        ...
        sim = SIM_CLASS(rng =rng, sel_rng=sel_rng, OneSq_cat=z_red, frac_ex=0.5, shear_population=0, **CONFIG)
        ...
        if match:
            gal0 = make_catalog(md.result, truth_cat, 0)       
            pres0 = _meas_shear(gal0)
            
            gal1 = make_catalog(md.result, truth_cat, 1)
            pres1 = _meas_shear(gal1)
            
            gal2 = make_catalog(md.result, truth_cat, 2)       
            pres2 = _meas_shear(gal2)
            
            gal3 = make_catalog(md.result, truth_cat, 3)       
            pres3 = _meas_shear(gal3)
            
            gal4 = make_catalog(md.result, truth_cat, 4)       
            pres4 = _meas_shear(gal4)        
        else:
            pes = _meas_shear(md.result)
        ...
        if match:
            gal0 = make_catalog(md.result, truth_cat, 0)       
            mres0 = _meas_shear(gal0)
            
            gal1 = make_catalog(md.result, truth_cat, 1)
            mres1 = _meas_shear(gal1)
            
            gal2 = make_catalog(md.result, truth_cat, 2)       
            mres2 = _meas_shear(gal2)
            
            gal3 = make_catalog(md.result, truth_cat, 3)       
            mres3 = _meas_shear(gal3)
            
            gal4 = make_catalog(md.result, truth_cat, 4)       
            mres4 = _meas_shear(gal4)
            
            retvals = (pres0, mres0, pres1, mres1, pres2, mres2, pres3, mres3, pres4, mres4)
        else:
...
if match:
    pres0, mres0, pres1, mres1, pres2, mres2, pres3, mres3, pres4, mres4 = zip(*outputs)
    pres0, mres0 = cut_nones(pres0, mres0)
    pres1, mres1 = cut_nones(pres1, mres1)
    pres2, mres2 = cut_nones(pres2, mres2)
    pres3, mres3 = cut_nones(pres3, mres3)
    pres4, mres4 = cut_nones(pres4, mres4)
    if rank == 0:
        ...
        m, msd, c, csd = estimate_m_and_c(pres2, mres2, 0.02, swap12=SWAP12)
        print("""\
        # of sims: {n_sims}
        noise cancel m   : {m:f} +/- {msd:f}
        noise cancel c   : {c:f} +/- {csd:f}""".format(
        ...
        m, msd, c, csd = estimate_m_and_c(pres3, mres3, 0.02, swap12=SWAP12)
        print("""\
        # of sims: {n_sims}
        noise cancel m   : {m:f} +/- {msd:f}
        noise cancel c   : {c:f} +/- {csd:f}""".format(
        ...
        m, msd, c, csd = estimate_m_and_c(pres4, mres4, 0.02, swap12=SWAP12)
        print("""\
        # of sims: {n_sims}
        noise cancel m   : {m:f} +/- {msd:f}
        noise cancel c   : {c:f} +/- {csd:f}""".format(
